...
a) bad commenting: this is a css comment and content shows html/js code
 ...
 #Answer
  1. if logic layer is javascript and presentation is HTML, it is bad to mix the logic layer and the presentation layer in one sentence. Each one of them must be separated:
...
#Answer
  ...
  b) there is an exception to the rule in the case of javascript. To increase the speed of the pages, HTML5 complains in the w3c indicates that the javascript code must go before the closure of the <body> tag.
  an example of that if twitter's bootstrap framework where personal js definitions are always defined on the footer.
